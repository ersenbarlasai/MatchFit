-- Coach Admin Role Check Fix Migration
-- Date: 2026-05-11
-- Description: Normalizes admin role checks using COALESCE, LOWER, and TRIM to ensure schema drift resilience and robust authority verification.

-- 1. Updated RPC: Handle Coach Verification (Normalized Role Check)
...
BEGIN
    -- Normalized Authorization check (admins or service role)
    IF COALESCE(auth.role(), '') != 'service_role'
       AND NOT EXISTS (
         SELECT 1
         FROM public.profiles
         WHERE id = auth.uid()
           AND LOWER(TRIM(COALESCE(role, ''))) IN ('admin', 'system_admin')
       )
    THEN
...
-- 2. Updated RPC: Handle Coach Document Review (Normalized Role Check)
